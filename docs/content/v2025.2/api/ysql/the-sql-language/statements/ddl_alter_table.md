...
Reusing a compatible index, and the notice raised on index rewrite, are available in {{<release "2025.2.3.0">}} and later. In earlier releases, an index that includes the altered column is always rewritten, and no notice is raised unless the main table is also rewritten.
