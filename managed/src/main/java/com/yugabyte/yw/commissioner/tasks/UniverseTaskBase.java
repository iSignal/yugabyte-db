...
public abstract class UniverseTaskBase extends AbstractTaskBase {
  ...
  private Set<UUID> lockedUniversesUuid = new HashSet<>();
  ...
  private Universe lockUniverseForUpdate(int expectedUniverseVersion, UniverseUpdater updater) {
    ...
    lockedUniversesUuid.add(taskParams().universeUUID);
    log.trace(
  ...
  public Universe lockUniverseForUpdate(UUID universeUuid, int expectedUniverseVersion) {
    UUID tempUniverseUuid = taskParams().universeUUID;
    taskParams().universeUUID = universeUuid;
    UniverseUpdater updater =
        getLockingUniverseUpdater(expectedUniverseVersion, true, false, false);
    Universe universe = lockUniverseForUpdate(expectedUniverseVersion, updater);
    taskParams().universeUUID = tempUniverseUuid;
    return universe;
  }
  ...
  public Universe unlockUniverseForUpdate(String error) {
    ...
    lockedUniversesUuid.remove(taskParams().universeUUID);
  ...
  /**
   * It creates subtasks to delete a set of tables in one universe.
   *
   * <p>Note: It creates the subtasks in sequence because the coreDB might not support deleting
   * several tables in parallel.
   *
   * @param universe The universe to delete the tables from
   * @param keyspaceTablesMap A map from keyspace name to a list of tables in that keyspace to be
   *     deleted
   * @param subTaskGroupType The subtask group that the created subtasks belong to
   */
  public void createDeleteTableFromUniverseTasks(
      Universe universe,
      Map<String, List<String>> keyspaceTablesMap,
      SubTaskGroupType subTaskGroupType) {
    for (Map.Entry<String, List<String>> keyspaceTablesEntry : keyspaceTablesMap.entrySet()) {
      keyspaceTablesEntry
          .getValue()
          .forEach(
              table -> {
                DeleteTableFromUniverse.Params deleteTableParams =
                    new DeleteTableFromUniverse.Params();
                deleteTableParams.universeUUID = universe.getUniverseUUID();
                deleteTableParams.masterAddresses = universe.getMasterAddresses();
                deleteTableParams.keyspace = keyspaceTablesEntry.getKey();
                deleteTableParams.tableName = table;
                createDeleteTableFromUniverseTask(deleteTableParams)
