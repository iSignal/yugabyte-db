...
Status CDCSDKYsqlTest::WaitForFlushTables(
  ...
  auto& client = *test_client();
  auto timeout = MonoDelta::FromSeconds(timeout_secs);
  boost::function<Status(void)> operation;
  if (is_compaction) {
    operation = [&client, &table_ids, timeout, add_indexes]() {
      return client.CompactTables(table_ids, timeout, add_indexes);
    };
  } else {
    operation = [&client, &table_ids, timeout, add_indexes]() {
      return client.FlushTables(table_ids, timeout, add_indexes);
    };
  }

  return RetryOnInternalError(
      std::move(operation), timeout,
      Format("Waiting for $0 operation to complete", is_compaction ? "compact" : "flush"));
}
